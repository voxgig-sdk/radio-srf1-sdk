// Command RadioSrf1-cli is a CLI + REPL for the RadioSrf1 SDK,
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      RADIO_SRF1_APIKEY (api key), RADIO_SRF1_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: RADIO_SRF1_APIKEY carries the API key and
	// RADIO_SRF1_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("RADIO_SRF1_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("RADIO_SRF1_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewRadioSrf1SDK(opts)
