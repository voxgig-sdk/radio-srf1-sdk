...
AQL-driven command-line client **and** interactive REPL for the RadioSrf1
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/radio-srf1-cli)
make build
...
RADIO_SRF1_BASE=https://api.example.com ./radio-srf1-cli list music

# 6. No arguments -> interactive REPL
./radio-srf1-cli
radio-srf1> list music
radio-srf1> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/radio-srf1-cli
   ```
...
3. **Run a query.** Evaluate an AQL expression against the API (or run with no
   ...
   ```sh
   ./dist/*/radio-srf1-cli list music
   ```
...
Run with no arguments to open a REPL (prompt `radio-srf1>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/radio-srf1-cli`. |
...
(`./radio-srf1-cli <expr>`) and interactively (the REPL), and expressions compose the
