...
//     (\`new RadioSrf1SDK(...)\` / \`RadioSrf1SDK.test(...)\` become
//     \`RadioSrf1SDK.test({ entity: ... })\`), transpiled to JS, and run
...
// rewrite real client construction (\`new RadioSrf1SDK(...)\` and any
// \`RadioSrf1SDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+RadioSrf1SDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/RadioSrf1SDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
