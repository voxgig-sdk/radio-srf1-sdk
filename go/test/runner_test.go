...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("RADIOSRF1_TEST_LIVE") == "TRUE" ||
		os.Getenv("RADIOSRF1_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("RADIOSRF1_TEST_EXPLAIN"); explain != "" {
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.RadioSrf1SDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.RadioSrf1Error{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.RadioSrf1SDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.RadioSrf1Error{Msg: msg, Code: code}
}
