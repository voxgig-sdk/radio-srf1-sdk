...
class RadioSrf1RetryFeature extends RadioSrf1BaseFeature
{
    ...
    public function init(RadioSrf1Context $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (RadioSrf1Context $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(RadioSrf1Context $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(RadioSrf1Context $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
