...
// RadioSrf1 SDK primary utility test
...
class PrimaryUtilityTest extends TestCase
{
    ...
    private static function make_ctx_from_map(
        ?array $ctxmap,
        RadioSrf1SDK $client,
        RadioSrf1Utility $utility
    ): RadioSrf1Context {
        if ($ctxmap === null) {
            $ctxmap = [];
        }

        $ctx = new RadioSrf1Context($ctxmap, null);
    ...
    private static function fixctx(RadioSrf1Context $ctx, RadioSrf1SDK $client): void
    {
    ...
    private static function err_from_map(?array $m): ?RadioSrf1Error
    {
        if ($m === null) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new RadioSrf1Error($code, $msg);
    }
    ...
    public function test_make_error_no_throw(): void
    {
        ...
        $ctx->result = new RadioSrf1Result([
            'ok' => false,
            'resdata' => ['id' => 'safe01'],
        ]);
    ...
    public function test_feature_add_basic(): void
    {
        ...
        $feature = new RadioSrf1BaseFeature();
    ...
    {
        $hook_client = RadioSrf1SDK::test(null, null);
        $hook_utility = $hook_client->get_utility();
        $ctx = self::make_test_ctx($hook_client, $hook_utility);

        $called = false;
        $hook_feature = new class extends RadioSrf1BaseFeature {
            public $hook_fn = null;
            public function TestHook(RadioSrf1Context $ctx): void
            {
    ...
    {
        $init_client = RadioSrf1SDK::test(null, null);
        ...
        $init_called = false;
        $feature = new class extends RadioSrf1BaseFeature {
            public $init_fn = null;
    ...
    {
        $blocked_client = new RadioSrf1SDK([
            'system' => [
    ...
    public function test_make_result_no_result(): void
    {
        ...
        $utility = $client->get_utility();
        $ctx = self::make_test_full_ctx($client, $utility);
        $ctx->spec = new RadioSrf1Spec(['step' => 'start']);
        $ctx->result = null;
    ...
    public function test_make_spec_basic(): void
    {
        ...
        $spec_client = RadioSrf1SDK::test(null, $setup_opts);
    ...
    public function test_prepare_auth_basic(): void
    {
        ...
        $auth_client = RadioSrf1SDK::test(null, $setup_opts);
    ...
    public function test_result_basic_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'resultBasic', 'basic'), function (array $entry) use ($client, $utility) {
            ...
            if ($result->err !== null) {
                $err_msg = ($result->err instanceof RadioSrf1Error) ? $result->err->msg : (string)$result->err;
