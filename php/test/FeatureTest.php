...
class FtCtrl extends RadioSrf1Control
{
...
class FtClient
{
    ...
    private RadioSrf1Utility $utility;

    public function __construct(RadioSrf1Utility $utility, array $options)
    {
    ...
    public function get_utility(): RadioSrf1Utility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (RadioSrf1Context $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public RadioSrf1Context $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (RadioSrf1Context $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = RadioSrf1Config::make_feature($name);
    ...
    private static function buildUrl(RadioSrf1Spec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof RadioSrf1Error) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new RadioSrf1Response($fetched);
        }
    ...
    private function fail(RadioSrf1Context $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (RadioSrf1Context $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
