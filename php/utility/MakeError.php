...
class RadioSrf1MakeError
{
    public static function call(?RadioSrf1Context $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            ...
            $ctx = new RadioSrf1Context([], null);
        }
        $op = $ctx->op ?? new RadioSrf1Operation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new RadioSrf1Result([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof RadioSrf1Error) ? $err->msg : (string)$err;
        ...
        $sdk_err = new RadioSrf1Error('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof RadioSrf1Error) {
