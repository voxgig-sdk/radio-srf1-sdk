...
require_relative 'RadioSrf1_types'
...
class RadioSrf1SDK
  ...
  def initialize(options = {})
    ...
    feature_opts = RadioSrf1Helpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = RadioSrf1Helpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, RadioSrf1Features.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    RadioSrf1Utility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = RadioSrf1Helpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = RadioSrf1Helpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = RadioSrf1Helpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def direct(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = RadioSrf1Helpers.to_int(VoxgigStruct.getprop(fetched, "status"))
