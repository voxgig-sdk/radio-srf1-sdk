...
class MusicEntity
  ...
  def data_set(args)
    if args
      @_data = RadioSrf1Helpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = RadioSrf1Helpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Array<Music>, Array] the matching Music items; raises RadioSrf1Error on failure
