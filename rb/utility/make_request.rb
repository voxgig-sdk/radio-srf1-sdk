...
module RadioSrf1Utilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = RadioSrf1Response.new({})
    ...
    if fetch_err
      ...
      response = RadioSrf1Response.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = RadioSrf1Response.new(fetched)
    else
