...
[MCP](https://modelcontextprotocol.io) server exposing the RadioSrf1 SDK as
two agent tools — `radio-srf1_list` and `radio-srf1_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/radio-srf1-mcp)
make build

# 2. Provide credentials via the environment
export RADIO_SRF1_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user radio-srf1 \
  -- /absolute/path/to/radio-srf1-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./radio-srf1-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// radio-srf1_list: first page of records
{ "entity": "music" }
{ "entity": "music", "query": { } }

// radio-srf1_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/radio-srf1-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user radio-srf1 \
     -- "$PWD"/dist/*/radio-srf1-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `radio-srf1_list` and `radio-srf1_load` tools now appear
   in new sessions. Ask the agent to *"list music using radio-srf1"*
   and it calls `radio-srf1_list` with `{"entity":"music"}`.
...
```sh
export RADIO_SRF1_APIKEY=sk_live_xxx            # API key
export RADIO_SRF1_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `radio-srf1_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `radio-srf1_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `RADIO_SRF1_APIKEY` | API key sent with every request. |
| `RADIO_SRF1_BASE` | Optional override of the API base URL. |
...
```sh
./radio-srf1-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"radio-srf1_load","arguments":{"entity":"music","query":{"id":1}}}}'
```
