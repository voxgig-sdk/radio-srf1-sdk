...
MCP server exposing the RadioSrf1 SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `radio-srf1_list` | `entity`, optional `query` map | First page of records as JSON |
| `radio-srf1_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `radio-srf1_list` and
`radio-srf1_load` tools become available in new sessions.
