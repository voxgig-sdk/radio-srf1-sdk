...
// RadioSrf1 SDK utility: make_point

require_once __DIR__ . '/../core/Helpers.php';

class RadioSrf1MakePoint
{
    public static function call(RadioSrf1Context $ctx): array
    {
        if (isset($ctx->out['point'])) {
            ...
            if ($ctx->out['point'] instanceof RadioSrf1Error) {
        ...
        if (count($op->points) === 1) {
            ...
            foreach ($op->points as $p) {
                $point = $p;
                $select_def = RadioSrf1Helpers::to_map(\Voxgig\Struct\Struct::getprop($p, 'select'));
                $found = true;
            ...
            if ($reqselector) {
                $req_action = \Voxgig\Struct\Struct::getprop($reqselector, '$action');
                if ($req_action && $point) {
                    $point_select = RadioSrf1Helpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'select'));
