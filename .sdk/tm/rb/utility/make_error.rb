# RadioSrf1 SDK utility: make_error
...
module RadioSrf1Utilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = RadioSrf1Context.new({}, nil)
    end
    op = ctx.op || RadioSrf1Operation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || RadioSrf1Result.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(RadioSrf1Error) ? err.msg : err.to_s
    msg = "RadioSrf1SDK: #{opname}: #{errmsg}"
    ...
    sdk_err = RadioSrf1Error.new("", msg, ctx)
    sdk_err.result = ctx.utility.clean.call(ctx, result)
    sdk_err.spec = ctx.utility.clean.call(ctx, spec)
    sdk_err.code = err.code if err.is_a?(RadioSrf1Error)
