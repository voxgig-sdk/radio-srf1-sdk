...
from core.operation import RadioSrf1Operation
...
def _err_from_map(m):
    ...
    if msg == "":
        return None
    code = m.get("code", "")
    return RadioSrf1Error(code, msg)
...
class TestPrimaryUtility:
    ...
    def test_feature_init_basic(self):
        init_client = RadioSrf1SDK.test(None, None)
    ...
    def test_feature_init_inactive(self):
        ...
        class TestInitFeatureInactive(RadioSrf1BaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "nofeat"
                self.active = False
    ...
    def test_make_result_no_result(self):
        ...
        utility = client._utility
        ctx = _make_test_full_ctx(client, utility)
        ctx.spec = RadioSrf1Spec({"step": "start"})
        ctx.result = None
    ...
    def test_make_spec_basic(self):
        ...
        spec_client = RadioSrf1SDK.test(None, setup_opts)
    ...
    def test_prepare_auth_basic(self):
        ...
        auth_client = RadioSrf1SDK.test(None, setup_opts)
